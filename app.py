...
def main():
    ...
    #====diagrama barras con media  de novedades x municipio===
    st.header("Diagrama de barras con media") # encabezado de la seccion de diagrama de barras
    df_avg = df.groupby("nov")["mun"].mean().reset_index() # agrupar por nov y calcular la media de las columnas numéricas, reset index para que el resultado sea un dataframe y no una serie
    #---------Diagrama de barras con la media de novedades por municipio----------
    fig2 = px.bar(df_avg, 
                x="nov", # eje x
                y="mun", # eje y
                title="Media de Novedades por Municipio", # título de la gráfica
                color="mun", # color de las barras 
                ) # escala de colores https://plotly.com/python/bar-charts/
